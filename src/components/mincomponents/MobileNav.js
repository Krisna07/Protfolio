...
const MobileNav = ({ menuHover }) => {
  const [openMenu, setOpenMenu] = useState(false);

  const rotateTopBar = "transform rotate-[-135deg] absolute";
  const middleBar = "transform translate-x-40 ";
  const rotatelastBar = "transform rotate-[135deg] absolute";
  return (
    <nav
      className={`w-full transition-all  ease-in duration-700 flex items-center relative`}
    >
      ...
      <div
        className={`w-screen transition-all ease-in duration-300  h-screen  absolute top-10 ${
          openMenu
      ...
      >
        <div
          className={`overflow-hidden h-full bg-gray-800 top-0  z-50 absolute ${
        ...
        >
          <Link to="#about" smooth onClick={() => setOpenMenu(!openMenu)}>
            <li className={`p-2 mx-4  w-[content] font-bold relative `}>
              ...
              >
                <span className="text-red-400 mx-2 ">0.1</span> About
              </span>
            </li>
          </Link>
          <Link to="#projects" smooth onClick={() => setOpenMenu(!openMenu)}>
            ...
            >
              <span
                className={`${menuHover} hover:text-red-400 flex items-center   `}
              >
                <span className="text-red-400 mx-2">0.2</span> Projects
              </span>
            </li>
          </Link>
          <li
